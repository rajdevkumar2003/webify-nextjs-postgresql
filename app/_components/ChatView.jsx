...
const ChatView = () => {
  ...
  const GetAiResponse=async()=>{
    setLoading(true);
    const PROMPT=JSON.stringify(messages)+'You are a AI Assistant and experience in React Development. GUIDELINES:Tell user what your are building response less than 15 lines. Skip code examples and commentary'
           ...
           {messages?.length>0&&messages.map((msg,index)=>(
              <div key={index} className="flex gap-2 w-full items-center justify-start  bg-[#688ccf]  rounded-sm shadow-sm mb-1">
                ...
                <Image
                    ...
                    alt="hero"
                />
                <h2 className="text-blue-950 font-semibold"> {msg.content}</h2>
                </div>
                :<>
                <p className="text-blue-950 p-1"> {msg.content}</p>
                </>}
              </div>
           ))}
           {loading&&<div className="flex gap-1 p-1 items-center justify-start text-sm"><LoaderCircleIcon className="animate-spin" />Working on your prompt...</div>}
        </div>
        <div className="relative flex w-full max-w-2xl min-h-52 rounded-md bg-[#86aef8] shadow-sm ">
          <textarea
            value={prompt}
